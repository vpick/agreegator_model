...
class NimbusApp implements AppOrderProcessInterface
{
    public function app_login_bkp($mapArray)
	{
	    #dd($mapArray['user_name']);
	 ...
	 public function app_login($mapArray)
	{
        ...
        curl_setopt_array($curl, array(
          ...
          CURLOPT_POSTFIELDS =>$loginNimbus,
	 ...
	 public function processOrder($shipment,$mapArray)
    {
		...
		if(!empty($shipment))
		{
			$auth = $this->app_login($mapArray);
		    $resultAuth = json_decode($auth, true);
			if($resultAuth['status'])
			{
			    
				$boxInOrder =  $shipment->input('no_of_boxes');
        		$boxTotalWeight = ($shipment->input('total_weight'))/1000;
				$productTotalWeight = array_sum(array_column($shipment->box_detail,'weight'));
				$productTotalBox = 0;
				if($boxInOrder>1)
				{
				    $productTotalBox = count($shipment->box_detail);;
				}
				else
				{
				    $productTotalBox = 1;
				...
				if($shipment->input('payment_mode') == "Prepaid" || strtolower($shipment->input('payment_mode')) == "prepaid")
				{
        		...
        		if(($productTotalBox == $boxInOrder) && ($boxTotalWeight == $productTotalWeight))
				{
        			if(($boxInOrder > 1) || ($boxTotalWeight/1000 > 20))
    				{
						...
						$pickupAddress['name'] = substr($shipment->input('origin.origin_name'), 0, 15);
                        ...
                        if($responseArray['status'] == 'true')
                        {
                            ...
                            $dataList['order_no'] = $shipment->input('order_no');
                            $dataList['shipping_label'] = $responseArray['data']['label'];
                            $dataList['awb_no'] = $responseArray['data']['awb_number'];
                            $dataList['msg'] = $responseArray['status'];
                            $dataList['courier_id'] =$responseArray['data']['courier_id'];
                            $dataList['courier_name'] = $responseArray['data']['courier_name'];
                            $dataList['packing_slip'] = $responseArray['data']['label'];
                        }
                        else
                        {
                            $status='failed';
                            ...
                            $dataList['awb_number'] = '';
                            $dataList['order_no'] = $shipment->input('order_no');
                            $dataList['shipping_label'] = '';
                            $dataList['message'] = $responseArray['message']?$responseArray['message'] :'';
                            $dataList['courier_id'] = 0;
                            $dataList['courier_name'] = '';
                            $dataList['packing_slip'] = '';
                        }
                       
                        #$dataList['status'] = $responseArray['status'];
    				...
    				else
    				{
    					...
    					if($responseArray['status'] == 'true')
    					{
                            ...
                            $dataList['shipping_label'] = $responseArray['data']['label'];
                            $dataList['awb_no'] = $responseArray['data']['awb_number'];
                            $dataList['message'] = $responseArray['status'];
                            $dataList['courier_id'] =$responseArray['data']['courier_id'];
                            $dataList['courier_name'] = $responseArray['data']['courier_name'];
                            $dataList['packing_slip'] = $responseArray['data']['label'];
                        }
		...
		{
			$result['message'] = 'Shipment detail not found..!';
			$result['status'] = false;
			return $result;
		}
    }
    public function processOrder_bkp($shipment,$mapArray)
    {
        #echo "NimbusApp";die;
		// send data to nimbuspost using cUrl
		if(!empty($shipment))
        		...
        		if(($productTotalBox == $boxInOrder) && ($boxTotalWeight == $productTotalWeight))
				{
    				...
    				else
    				{
    					...
    					if($responseArray['status'] == 'true'){
                            ...
                            $dataList['order_no'] = $shipment->input('order_no');
                            $dataList['shipping_label'] = '';
                            $dataList['msg'] = $responseArray['message']?$responseArray['message'] :'';
                            $dataList['courier_id'] = 0;
                            $dataList['courier_name'] = '';
                            $dataList['packing_slip'] = '';
                        }
    ...
    public function processShipOrder($shipmentDetail,$mapArray){
		...
		if(!empty($shipment))
		{
			...
			if($resultAuth['status'])
			{
                ...
                $response = curl_exec($curl);
                $courier_list = json_decode($response, true);
                $courier_id = '';
                foreach ($courier_list['data'] as $key => $list) {
                    if ($list['name'] === 'Xpressbees') {
                        $courier_id = $key;
        		...
        		if(($productTotalBox == $boxInOrder) && ($boxTotalWeight == $productTotalWeight))
				{
    				...
    				else
    				{
    					...
    					if($result['status'] == "true"){
        					...
        					$data['awb_number'] = $result['data']['awb_number']?$result['data']['awb_number']:'';
                           
                            $data['order_no'] = $shipment['order_no'];
                            $data['shipping_label'] = $result['data']['label']?$result['data']['label']:'';
                            $data['message'] = $result['data']['status'] ? $result['data']['status'] :'';
                            $data['courier_id'] = $result['data']['courier_id'] ? $result['data']['courier_id'] : 0;
                            $data['courier_name'] = $result['data']['courier_name'] ? $result['data']['courier_name'] :'';
                            $data['packing_slip'] = $result['data']['label']?$result['data']['label']:'';
    					}
    					else{
                            ...
                            $data['courier_name'] = $shipment['courier_name'];
                            $data['packing_slip'] = '';
                            $data['status'] = 'failed';
    ...
    public function reprocessOrder_bkp($shipment,$mapArray)
    {
        echo "NimbusApp";die;
		// send data to nimbuspost using cUrl
		if(!empty($shipment))

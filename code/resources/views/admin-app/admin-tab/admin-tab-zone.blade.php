...
<section class="py-filter">
	<div class="col-md-12 text-right">
		<a href="{{ route('zone.index') }}" class="btn btn-sm btn-outline-dark refresh_orders_button"><i class="mdi mdi-refresh"></i> Refresh</a>		
		<button class="btn btn-outline-dark btn-sm dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false">Create</button>
    	    <ul class="dropdown-menu shadow-sm">
    			<li><a class="dropdown-item" href="{{ route('zone.create') }}"><i class="mdi mdi-plus"></i> Add Zone</a></li>
    	    </ul>
	...
	<div id='filter' style='display:none'>
        <div class="card-body">
    		<form class="row g-3 align-items-center" method="GET" action="/zone">	
    		<div class="row">
    			<div class="form-group col-lg-2">
    				<label>DSP</label>
    				 @php
    			        $selectedDsp =  request()->input('dsp');
    			    @endphp
    				<select class="multiple-select" id="dsp" name="dsp[]" multiple="multiple" placeholder="dsp">
    				    @foreach($couriers as $courier)
    						<option value="{{ $courier->id }} " {{ $selectedDsp ? ((in_array($courier->id, $selectedDsp)) ? 'selected' : '') : '' }}>{{ $courier->logistics_name }}</option>
    					@endforeach
    				</select>
    			...
    			<div class="col-lg">
    				...
    				<a href="/zone" class="btn btn-secondary" style="margin-top: 25px;border-radius: 5px">Clear</a>
    			</div>
    		</div>
    		</form>
    	</div>
			            ...
			            <table class="table align-middle mb-0 bg-white">
			                ...
			                <tbody>
                			 @foreach($zones as $zone)
								<tr>
            				        <td>
            							<a title='View User' class="btn btn-sm btn-outline-primary" style="padding-left: 7px;padding-right: 0px;" href="{{ route('zone.edit', \Crypt::encrypt($zone->id)) }}">
            								<svg class="svg-icon svg-icon-sm svg-icon-heavy me-2 img-icon"><use xlink:href="#survey-1"> </use></svg>
            							</a>														
            						</td>
            						<th>{{ $zone->courier->logistics_name }}</th>

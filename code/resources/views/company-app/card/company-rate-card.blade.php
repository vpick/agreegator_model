                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                                ...
                                <div class="row mb-3">
                                    <div class="col-lg-2 mb-2">
                                        ...
                                        <select class="form-control" id="contract_type" name="contract_type" required>
                                            ...
                                            <option value="company_client" selected>Company to Client</option>    
                                            <option value="company_dsp">Company to DSP</option>
                                            @else
                                            <option value="client_dsp">Client to DSP</option>
                                            @endif
                                        </select>
                                        @error('contract_type')
                            ...
                            <div class="col-lg-12">                 
                                <div class="row py-3" id="tableZone">
                        ...
                        <div class="row">
                            <div class="col-lg-4">
                                <button class="btn btn-primary" type="submit" id="submitBtn" @if(empty($zones)) disabled @endif>Submit</button>
                                <a href="{{ route('rate-card.index') }}" class="btn btn-danger" id="cancelBtn">Cancel</a>
                            </div> 
                        </div>
                    </form>
                </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#contract_type').on('change', function() {
            ...
            var logistics_partner = $('#logistics_partner').val();
            ...
            if(contract_type!='')
            {
                ...
                {
                    $.ajax({
                        url: '/load/zone_list/' + contract_type,
                        type: 'GET',
                        success: function(res) { 
                            ...
                            {
                                cont += `<div class="table-responsive" style="border: 1px solid #e4e4e4;"><table class="table" style="margin-bottom: 5px!important;"><thead class="thead-dark"><th class="text-center" style="width:2%!important">#<br><span class="fs" style="color:transparent">#</span></th>`
                                $.each(res.data, function(index, val) {                        
                                    cont += `<th class="text-center"><p class="p-css">${val['zone_code']}</p><span class="fs">${val['description']}</span></th>`;
                                });
                                cont +=`</thead><tbody><tr><td><p class="bg-css mb-2">Forward Rate</p></td>`;      
                                $.each(res.data, function(index, val) {                        
                                    cont += `<td class="text-center"><input type="text" class="form-control mb-2" id="forward_rate" value="{{ '0.00' }}" name="forward_rate[]"></td> `;
                                });                                                          
                                cont +=`<tr><td><p class="bg-css mb-2">FWD - Additional Rate</p></td>`;
                                $.each(res.data, function(index, val) {                        
                                    cont += `<td class="text-center"><input type="text" class="form-control mb-2" id="forward_additional_rate" value="{{'0.00' }}" name="forward_additional_rate[]"></td> `;
                                });                                                       
                                cont +=`<tr><td><p class="bg-css mb-2">Reverse Rate</p></td>`;
                                $.each(res.data, function(index, val) {                        
                                    cont += `<td class="text-center"><input type="text" class="form-control mb-2" id="reverse_rate" value="{{ '0.00' }}" name="reverse_rate[]"></td> `;
                                });                                                   
                                cont += `</tr><tr><td><p class="bg-css mb-2">DTO Rate</p></td>`;
                                $.each(res.data, function(index, val) {                        
                                    cont += `<td class="text-center"><input type="text" class="form-control mb-2" id="dto_rate" value="{{ '0.00' }}" name="dto_rate[]"></td>`;
                                });
                                cont +=`</tr></tbody></table></div>`;
                                $('#tableZone').append(cont);
        ...
        $('#logistics_partner').on('change', function(){
            $('#courier_id').val('');
            var dsp = $(this).val();
            var contract_type = $('#contract_type').val();
            $.ajax({
                url: '/load/zone_dsp_list/' + contract_type + '/' + dsp,
                type: 'GET',
                success: function(res) { 
                    $('#tableZone').empty();    
                    var cont ='';

...
<section class="py-filter">
    <div class="row">
        <div class="col-md-6 text-left"> 
            ...
            <small>By default 1 month data appear {{Auth::user()->user_type }}</small>
        </div>
		...
		@endif
		<a href="/orders" class="btn btn-sm btn-outline-dark refresh_orders_button"><i class="mdi mdi-refresh"></i> Refresh</a>
        ...
        @endphp
		    <ul class="nav nav-tabs" id="myTab" role="tablist" >
			  ...
			  <li class="nav-item me-2" role="presentation">
				...
				<a class="nav-link btn-css  {{ $ord ? 'btn-white' : 'activeClass btn-black'}}" href="{{ url('/orders') }}" id="contact-tab" data-bs-target="#home-tab-pane" type="button" role="tab" aria-controls="contact-tab-pane" aria-selected="false">All Shipments ( {{ $total_shipment  }} )</a>
			  </li>
			  <li class="nav-item me-2" role="presentation">
			
				<a class="nav-link btn-css  {{ $ord=='Booked,ship' ? 'activeClass btn-black':'btn-white' }}" href="{{ url('/orders?ord=Booked,ship') }}" id="contact-tab" data-bs-target="#profile-tab-pane" type="button" role="tab" aria-controls="contact-tab-pane" aria-selected="false">New Shipments ({{ $book }})</a>
			  </li>
			 <!-- <li class="nav-item me-2" role="presentation">-->
			 ...
			 <!-- </li>-->
			  <li class="nav-item me-2" role="presentation">
				<a class="nav-link btn-css {{ $ord=='Pending Pickup' ? 'activeClass btn-black':'btn-white' }}" href="{{ url('/orders?ord=Pending Pickup') }}" id="contact-tab" data-bs-target="#contact-tab-pane" type="button" role="tab" aria-controls="contact-tab-pane" aria-selected="false">Pending Pick ({{ $pick }})</a>
			  </li>
	      <!--    <li class="nav-item me-2" role="presentation">-->
    		 ...
    		 <!-- </li>-->
    		  <li class="nav-item me-2" role="presentation">
    			<a class="nav-link btn-css {{ $ord=='In Transit' ? 'activeClass btn-black':'btn-white' }}" href="{{ url('/orders?ord=In Transit') }}" id="contact-tab" data-bs-target="#profile-tab-pane" type="button" role="tab" aria-controls="contact-tab-pane" aria-selected="false">In-Transit ({{ $transit }})</a>
    		  </li>
    		  <li class="nav-item me-2" role="presentation">
    			<a class="nav-link btn-css {{ $ord=='Out for Delivery' ? 'activeClass btn-black':'btn-white' }}" href="{{ url('/orders?ord=Out for Delivery') }}" id="contact-tab" data-bs-target="#contact-tab-pane" type="button" role="tab" aria-controls="contact-tab-pane" aria-selected="false">Out for Delivery ({{ $out_for_delivery }})</a>
    		  </li>
    		  <li class="nav-item me-2" role="presentation">
    			<a class="nav-link btn-css {{ $ord=='Delivered' ? 'activeClass btn-black':'btn-white' }}" href="{{ url('/orders?ord=Delivered') }}" id="contact-tab" data-bs-target="#profile-tab-pane" type="button" role="tab" aria-controls="contact-tab-pane" aria-selected="false">Delivered ({{ $delivered }})</a>
    		  </li>
    		  <li class="nav-item me-2" role="presentation">
    		      <button class="btn btn-css dropdown-toggle " type="button" data-bs-toggle="dropdown" aria-expanded="false" style="margin-top: 5px;background: white;border-radius: 5px;color: #12263f!important;">@if($ord == 'RTO') RTO Shipments @elseif($ord == 'Cancelled') Cancelled @else More @endif</button>
                    <ul class="dropdown-menu shadow-sm">
                        <li class="nav-item me-2" role="presentation">
                			<a class="nav-link btn-css {{ $ord=='RTO' ? 'activeClass btn-black':'btn-white' }}" href="{{ url('/orders?ord=RTO') }}" type="button">RTO Shipments ({{ $rto }})</a>
                		  </li>
                		  <li class="nav-item me-2" role="presentation">
                			<a class="nav-link btn-css {{ $ord=='Cancelled' ? 'activeClass btn-black':'btn-white' }}" href="{{ url('/orders?ord=Cancelled') }}" type="button">Cancelled ({{ $cancelled }})</a>
                		  </li>
                		   <li class="nav-item me-2" role="presentation">
                			<a class="nav-link btn-css {{ $ord=='Failed' ? 'activeClass btn-black':'btn-white' }}" href="{{ url('/orders?ord=Failed') }}" type="button">Failed Shipments ({{ $failed }})</a>
                		  </li>
                    </ul>
    		  </li>
                    			 ...
                    			 <!-- </td>-->
                    			  ...
                    			  <td>
                    				@if($order->order_status == 'Booked' && $order->status=='0')
                    				    ...
                    				    <input type="hidden" id="volumetric_weight{{ $order->order_no}}" value="{{$order->volumetric_weight }}">
                    					<p class="fw-normal mb-1"><span class="btn btn-outline-dark btn-sm popUp" style="width: 90%!important;"  id="{{ $order->order_no}}">{{ucwords($order->order_status)}}</span></p>
                    			  	@elseif($order->order_status == 'ship')
                    			  	    <p class="fw-normal mb-1"><span class="btn {{ $btn }} btn-sm ship_order" style="width: 90%!important;" id="{{ $order->order_no}}">{{ucwords($order->order_status)}}</span></p>
                    			  	@else
            ...
            <form action='/update-order' method="POST">
                ...
                <div class="modal-header">
                    ...
                    @if(Auth::user()->user_type = "isClient")
						<span>Switch for DSP Contract  <input type="checkbox" id="configType" name="client_dsp" class="js-switch" ></span>            
					@endif	
					</div>
			    </div>
                <div class="modal-body">
						...
						<div class="col-sm-1">
							<span style="padding-top: 0px;"><a href="{{ url('pdf/dangers_goods.pdf') }}" target="_blank" title="material safety data sheet" style="color:darkgray;"><img src="{{ url('pdf.png') }}" style="height:35px;"></a></span>
						</div>
                        <div class="col-sm-4">
                    	...
                    	<div class="row p-t-10  m-b-10">
                            ...
                            <div class="col-sm-5">
                                <div class="input-group">
                                    <div class="input-group-prepend" >
                                        <span class="input-group-text bg-span"style='border-radius: 4px 4px 4px 4px !important'> 
                                      
                                         	<input type="radio" id="shipment_type{{$key}}" name="shipment_type"  value="All" class="shipment_type btn btn-outline-secondary filter mr-2">Shipment Type
										</span>
                                    </div>
                                    <div class="input-group-append btn-group-toggle" data-toggle="buttons">
...
$(document).ready(function () { 
	...
	$(".popUp").click(async function (event) {
	    var configElement = $("#configType").siblings('span');
		var configSmall = configElement.children('small');
		...
		configSmall.css({
			...
			total_price = $('#total_price' + order_no).val();	
			...
			pincode = $('#pincode' + order_no).val();
			payment_mode = $('#payment_mode' + order_no).val();
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if(response.data!='')
                        {
                            ...
                            var content = '';
                            var resultData = [];  // Assume resultData is an empty array
                            resultData = response.data;
                            var sortedData = resultData.length > 0 ? resultData.sort(function (a, b) {
	...
	$("#configType").change(function () {
        ...
        {   
            var weightRange = $('input[name="weight_range"]:checked').val();
            var contract_type = $(this).attr('name');
            $('#shipBtn').attr('disabled', false);
            $('#courier_rate').text('');
            $("#myModal").modal('show');
            try 
